import { UnstyledButton, TextInput } from "@mantine/core";
...
export function JournalList({ positions, selected, onSelect }) {
  ...
  const visible = positions.filter((p) => {
    ...
    return (!account.length || account.includes(p.account_id))
      && `${p.ticker} ${p.instrument_name}`.toLowerCase().includes(searchTerm)
  ...
  return (
    <section className="journal-list" aria-label="Позиции для разбора">
      <div className="journal-filters">
        <TextInput aria-label="Поиск инструмента" type="search" placeholder="Тикер или инструмент" value={search} onChange={(e) => setSearch(e.target.value)} />
        <AccountPicker accounts={accounts.map(([id, name]) => ({ id, name }))} value={account} onChange={setAccount} />
        <FilterSelect aria-label="Заполнение дневника" value={review} onChange={setReview} placeholder="Все позиции" data={[{"value": "empty", "label": "Без заметок"}, {"value": "filled", "label": "Есть заметки"}]} />
      </div>
      <div className="journal-items">
        {visible.map((p) => <UnstyledButton type="button" key={p.id} className={`journal-item ${selected === p.id ? "active" : ""}`} onClick={() => onSelect(p.id)} aria-pressed={selected === p.id}>

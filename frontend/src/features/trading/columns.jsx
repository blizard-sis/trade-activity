...
export function createColumns() {
  return [
    ...
    { key: "account", label: "Платформа — счёт", sort: "account", align: "text-left", render: (item) => item.account_name },
    ...
    { key: "gross_result", label: "Результат", render: (item) => formatMoney(item.gross_result, item.result_currency) },
    { key: "commission", label: "Комиссия", render: (item) => formatMoney(item.commission, item.commission_currency) },
    { key: "net_result", label: "Чистыми", render: (item) => <span className={resultClass(item.net_result)}>{formatMoney(item.net_result, item.result_currency)}</span> },
    ...
    { key: "journal", label: "Дневник", align: "text-left", render: (item) => <a className="journal-link" href={`/journal?position=${encodeURIComponent(item.id)}`}>Открыть разбор ↗</a> },
  ];
}
